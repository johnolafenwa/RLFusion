"""SFT on johnolafenwa/reasoning-sft with Qwen/Qwen3-4B-Instruct-2507.

The dataset already contains the target response format:
  <think>...</think> \boxed{answer}

Use this checkpoint as the starting point for
`examples/grpo_qwen3_4b_instruct_2507.py`.
"""
...
class ReasoningSFTDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> ReasoningSFTEnv:
        ...
        if user_input is None:
            raise ValueError("Dataset row missing required field: input.")
        if assistant_output is None:
            raise ValueError("Dataset row missing required field: output.")

        return ReasoningSFTEnv(
            prompt=[{"role": "user", "content": str(user_input)}],
            answer=str(assistant_output),
        )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Qwen3-4B-Instruct-2507 on johnolafenwa/reasoning-sft."
    )
    parser.add_argument("--model", type=str, default="Qwen/Qwen3-4B-Instruct-2507")
    parser.add_argument("--output-dir", type=str, default="./outputs/qwen3_4b_instruct_2507_reasoning_sft")
